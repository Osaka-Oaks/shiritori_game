# GitHub templates (markdown flavor)
...
1. **New issue** → choose a template from the issue picker, **or**
2. Copy the body from a file above into a blank issue/PR.

## Labels & kanban

See [KANBAN.md](../KANBAN.md) and [labels.yml](../labels.yml).

**Status:** `status: triage` → `backlog` → `ready` → `in progress` → `review` → `done`

**Areas:** `area: shiritori-online`, `area: kawaii-shiritori`, `area: flutter`, `area: ci/cd`, `area: infra`
