...
import { DICTIONARY } from "../dictionary/dictionary-data";
import { WORD_BANK } from "../dictionary/word-bank";
import { type LevelConfig, jlptAllowed } from "./levels";
import type { DevSettings } from "../dev/dev-mode";
...
function buildVocab(level: LevelConfig): BotWord[] {
  ...
  return [...map.values()].filter(
    w => w.kana.length >= 2 && getChainKana(w.kana) !== "ん" && jlptAllowed(w.jlpt, level.maxJlpt)
  );
}
...
function countFollowUps(kana: string, usedSet: Set<string>, vocab: BotWord[]): number {
