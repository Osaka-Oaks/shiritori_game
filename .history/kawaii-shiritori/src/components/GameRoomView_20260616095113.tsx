...
import JapaneseInputField from "./JapaneseInputField";
...
export default function GameRoomView({
  ...
  const handleWordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Validate word using local dictionary
      const wordEntry = dictionary.findWord(finalWordToCheck);

      setEvaluatingWord(false);

      if (!wordEntry) {
        triggerOopsModal(`"${finalWordToCheck}" is not recognized in our dictionary. Please try another word!`, finalWordToCheck);
        return;
      }
      ...
      const checkResult = {
        isValid: true,
        word: wordEntry.word,
        romaji: wordEntry.romaji,
        kanji: wordEntry.kanji,
        translation: wordEntry.translation,
        hiragana: wordEntry.word,
        startSound: wordEntry.startSound,
        endSound: wordEntry.endSound,
        endsInN: dictionary.endsInN(wordEntry.word)
      };
