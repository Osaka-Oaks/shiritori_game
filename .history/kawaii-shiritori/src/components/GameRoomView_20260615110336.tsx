...
export default function GameRoomView({
  ...
  const handleBotTurn = async (currentHistory: PlayedWord[]) => {
    ...
    try {
      ...
      const botDuplicate = currentHistory.some(
        w =>
      ...
      if (botDuplicate) {
        ...
        setBotChat(
          `Oh no! I accidentally repeated "${botPlay.word}". That's against the rules - you win!`
        );
  ...
  const handleRestartDuel = () => {
    ...
    setBotChat(
      `New game! I played "Ringo" (りんご) = 🍎 Apple. Match the sound "go" (ご)! がんばって!`
    );
  };
