...
export default function PlayRoom({
  ...
  const opponentName =
    opponentType === "mei" ? "Mei-chan" : opponentType === "neko" ? "Bot Neko" : "Inu Shiba";
  ...
  const triggerGameOver = (fatalWord: string, lastWordPlayed: string, winnerName?: string) => {
    ...
    setGameOverGameOverStats({
      ...
      reason:
        fatalWord === "Time Up"
          ? "You ran out of time!"
          : `${decidedWinner === "Player" ? opponentName : "You"} played a word ending with 'N' ("${fatalWord}")!`,
    });
  ...
  const useWordHint = async () => {
    ...
    try {
      ...
      if (res.ok && data.word) {
        setHintMessage(
          `HINT: Try word "${data.word}" (${data.hiragana}) which means "${data.meaning}"!`
        );
      } else {
      ...
      <div className="relative z-10 flex-grow flex flex-col w-full h-full max-w-4xl mx-auto px-4 py-3 pointer-events-none">
        ...
        <div className="flex justify-between items-start w-full pointer-events-auto">
          ...
          <div className="flex flex-col items-center gap-1.5 mt-1">
            <div
              className={`px-5 py-2 rounded-sm border flex items-center gap-2 text-white transition-all ${
                turn === "player"
            ...
            <div className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-3 py-1 rounded-sm text-xs font-mono font-bold text-[#C5A059]">
              <span
                className={`w-2 h-2 rounded-full bg-[#C5A059] ${timeLeft < 10 ? "animate-ping bg-red-400" : ""}`}
              ></span>
              <span className="text-[10px] tracking-widest font-light uppercase">
        ...
        {hintMessage && (
          <div className="mt-4 bg-white/[0.02] border border-[#C5A059]/30 text-white max-w-md mx-auto p-4 rounded-sm shadow-lg text-xs pointer-events-auto border-l-2 border-l-[#C5A059] animate-fadeIn flex items-center gap-3">
            <span className="font-bold uppercase tracking-widest border border-[#C5A059]/40 text-[#C5A059] px-2 py-0.5 rounded-sm shrink-0 text-[9px]">
              HINT
            </span>
            ...
            <div className="flex flex-col gap-4 relative z-10">
              {wordChain.map((item, idx) => (
                ...
                >
                  ...
                  <div
                    className={`w-8 h-8 rounded-full overflow-hidden border shrink-0 shadow-lg ${
                      item.speaker === "player" ? "border-[#C5A059]/40" : "border-white/20"
                    }`}
                  >
                    <img
                      className="w-full h-full object-cover"
                      src={item.speaker === "player" ? userAvatar : opponentAvatar}
                      alt="Mini Avatar"
                    />
                  </div>
                  ...
                  {/* Word item bubble */}
                  <div
                    className={`rounded-sm px-4 py-3 shadow-md border max-w-[210px] cursor-pointer transition-all ${
                      item.speaker === "player"
          ...
          <div className="flex gap-3 items-end relative w-full">
            ...
            <form onSubmit={handleWordSubmit} className="flex-1 relative">
              ...
              <input
                ...
                placeholder={
                  turn === "player" ? "Type ROMAJI or JAPANESE word..." : "Waiting for opponent..."
                }
                type="text"
                value={userWordInput}
      ...
      {showSuccessOverlay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn pointer-events-auto">
          <div className="glass-card rounded-sm p-8 flex flex-col items-center text-center shadow-2xl relative w-full max-w-sm mx-auto animate-fadeIn border-[#C5A059]/30">
            <span className="absolute top-4 left-4 text-[#C5A059] text-3xl opacity-30 animate-pulse">
              ★
            </span>
            <span className="absolute bottom-6 right-6 text-[#C5A059] text-2xl opacity-20 animate-bounce">
              ✿
            </span>
      ...
      {showOopsOverlay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn pointer-events-auto">
          <div className="glass-card rounded-sm p-8 flex flex-col items-center text-center shadow-2xl relative w-full max-w-sm mx-auto border-red-500/20">
            ...
            <p className="text-slate-300 font-light text-sm mb-6 leading-relaxed">{oopsReason}</p>
        ...
        <div className="fixed inset-0 z-50 bg-[#050505] flex flex-col items-center justify-center p-6 overflow-y-auto pointer-events-auto">
          <div className="max-w-md w-full flex flex-col gap-6 py-8">
            <div className="text-center">
              <h1 className="serif-italic text-4xl text-white font-light tracking-wide">
                Game Over
              </h1>
            ...
            <div className="glass-card rounded-sm p-6 shadow-xl flex flex-col gap-4 relative overflow-hidden">
              <div className="flex justify-between items-center pb-4 border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-[9px] uppercase tracking-widest text-white/40 font-bold">
                      WINNER
                    </h3>
                    <p className="serif-italic text-lg text-white font-medium">
                      {gameOverStats.winner}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <h3 className="text-[9px] uppercase tracking-widest text-[#C5A059]/80 font-bold">
                    YOUR SCORE
                  </h3>
                  <p className="serif-italic text-xl text-[#C5A059] font-medium">
                    {gameOverStats.playerScore}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-center bg-white/[0.01] rounded-sm p-3 border border-white/5">
                  <span className="text-xs font-light text-white/60 uppercase tracking-wide">
                    Word Chain Length
                  </span>
                  <span className="serif-italic text-sm text-white font-medium">
                    {gameOverStats.chainLength} Words
                  </span>
                </div>
                <div className="flex justify-between items-center bg-red-500/[0.02] rounded-sm p-3 border border-red-500/15">
                  ...
                  <span className="serif-italic text-sm text-red-400 capitalize">
                    {gameOverStats.fatalWord}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#050505]">
              {chatMessages.map(msg => (
                ...
                >
                  ...
                  <div
                    className={`px-4 py-2.5 shadow-sm max-w-[85%] rounded-sm ${
                      msg.isSticker
                  ...
                  >
                    {!msg.isSticker && (
                      <p className="text-xs font-light whitespace-pre-wrap">{msg.text}</p>
                    )}
