...
function provideRecommendations(app, stats, builds) {
  ...
  // High variance
  const variance = Math.max(...builds.map(b => b.timings.total)) - Math.min(...builds.map(b => b.timings.total));
  if (variance > 120) {
    recommendations.push({
      severity: 'low',
  ...
  if (recommendations.length === 0) {
    ...
    recommendations.forEach(rec => {
      const severityColor = rec.severity === 'high' ? 'red' : rec.severity === 'medium' ? 'yellow' : 'cyan';
      log(`\n  ${getSeverityIcon(rec.severity)} [${rec.severity.toUpperCase()}] ${rec.message}`, severityColor);
      log('  Actions:', 'bright');
