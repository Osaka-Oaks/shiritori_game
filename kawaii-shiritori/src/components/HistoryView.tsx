...
export default function HistoryView({ matches, onClearHistory, onSelectMatch }: HistoryViewProps) {
  ...
  const highestChain = matches.reduce(
    (acc, curr) => (curr.chainLength > acc ? curr.chainLength : acc),
    0
  );
    ...
    <div className="w-full max-w-2xl mx-auto py-4 px-4 pb-24 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-surface-container-low rounded-2xl p-4 border border-outline-variant/30 text-center shadow-sm flex flex-col items-center justify-center space-y-1.5">
          ...
          <span className="text-[10px] font-label-caps text-on-surface-variant/80">
            TOTAL SCORE
          </span>
        </div>
              ...
              >
                <div className="flex items-center gap-4 text-left">
                  <div className="relative">
                    ...
                    />
                    <span
                      className={`absolute -bottom-1 -right-1 text-xs p-1 rounded-full text-on-primary font-bold ${
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-on-surface-variant font-medium flex items-center gap-1 mt-0.5">
                      ...
                      {item.date} • Last word:{" "}
                      <span className="font-semibold text-primary font-display-game">
                        {item.fatalWord || "N/A"}
                      </span>
                    </p>
                  </div>
                ...
                <div className="text-right flex flex-col items-end gap-1">
                  ...
                  <div className="font-headline font-black text-sm tracking-tight text-on-surface group-hover:text-primary transition-colors">
                    {item.playerScore}{" "}
                    <span className="text-outline-variant font-medium text-xs">vs</span>{" "}
                    {item.opponentScore}
                  </div>
