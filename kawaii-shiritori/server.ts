...
// ===== OLLAMA AI ENDPOINTS (FREE LOCAL AI) =====

// 4. Ollama Validate Word
app.post("/api/ollama/evaluate-word", async (req, res) => {
  const { word } = req.body;
...
// 5. Ollama Opponent Turn
app.post("/api/ollama/opponent-turn", async (req, res) => {
  const { lastSound, difficulty, playedWords } = req.body;
  const OLLAMA_URL = process.env.OLLAMA_URL || "http://localhost:11434";
  const MODEL = process.env.OLLAMA_MODEL || "qwen3";
...
// 6. Ollama Word Hints
app.post("/api/ollama/word-hint", async (req, res) => {
  const { requiredSound, usedWords } = req.body;
  const OLLAMA_URL = process.env.OLLAMA_URL || "http://localhost:11434";
  const MODEL = process.env.OLLAMA_MODEL || "qwen3";

  const prompt = `Give 3 Japanese words that start with: ${requiredSound}

Exclude these already used: ${usedWords.join(", ")}

Each hint should include: word (hiragana) = English

Return JSON only:
{
  "hints": ["word1 (hiragana1) = meaning1", "word2 (hiragana2) = meaning2", "word3 (hiragana3) = meaning3"]
}`;
